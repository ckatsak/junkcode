...
def _inner_top_subdirs(start, end, index_height):
    # end_aux = end + (1 << 4 * index_height) if start > end else end
    if start > end:
...
def _inner_top_subdirs_middle(start, end, exp, index_height):
    ...
    first = n = (start / step + 1) * step

    # left:
    for i in _inner_top_subdirs_left(start, first, exp-1, index_height):
        yield i
...
def _inner_top_subdirs_right(right_limit, end, exp, index_height):
    ...
    print tab, "Entering _inner_top_subdirs_right(%s, %s, %d, %d)" % (
            hex(right_limit)[2:], hex(end)[2:], exp, index_height)
    if exp < 0:
        raise StopIteration
    ...
    while n <= end - step:
        print tab, hex(n)[2:].zfill(index_height+1)[1:], index_height-exp
        yield hex(n)[2:].zfill(index_height+1)[1:], index_height-exp
        n += step

...
pub trait Strategy: Debug {
    fn apply(&mut self, yolo: &mut Yolo) -> u64;
}
...
impl Strategy for S1 {
    fn apply(&mut self, yolo: &mut Yolo) -> u64 {
        self.count += 1;
        yolo.m.insert(self.count, format!("{}", self.count));
        self.s = format!("{}", yolo.m.len());
        self.count
    }
}
...
impl Strategy for S2 {
    fn apply(&mut self, yolo: &mut Yolo) -> u64 {
        self.count += 1;
        yolo.m.insert(self.count, format!("{}", 2 * self.count));
        self.s = format!("{}", 2 * yolo.m.len());
        2 * self.count
    }
}
...
impl Yolo {
    ...
    pub fn strategy(&mut self) -> u64 {
        let s = self.s.clone();
        let ret = s.borrow_mut().apply(self);
        ret
    }
}
